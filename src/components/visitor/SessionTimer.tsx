...
export function SessionTimer({ checkInAt, checkInLabel = "09:00 WIB", isCompleted = false }: SessionTimerProps) {
  ...
  useEffect(() => {
    if (!checkInAt || isCompleted) return;
  ...
  return (
    <div className="flex items-center gap-3 mt-4 fade-in-up" style={{ animationDelay: "0.3s" }}>
      <div className="bg-white/20 px-3 py-1.5 rounded-full flex items-center gap-1.5 backdrop-blur-sm">
        <div className={`w-2 h-2 rounded-full ${isCompleted ? "bg-white/40" : "bg-secondary-fixed animate-pulse"}`}></div>
        <span className="font-plus-jakarta-sans text-[10px] font-bold uppercase tracking-wider">
          {isCompleted ? "Kunjungan Selesai" : `Sesi Aktif (${elapsed})`}
        </span>
      </div>
